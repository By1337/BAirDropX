package org.by1337.bairx.hook.basehead;
...
public class BaseHeadHook {
    public static ItemStack getItem(String argument) {
        if (argument == null) {
            BAirDropX.getMessage().error("material is null!");
        ...
        if (argument.startsWith("basehead-")) {
            try {
                ...
                BAirDropX.getMessage().error("Something went wrong while trying to get base64 head: " + argument, exception);
                return new ItemStack(Material.PLAYER_HEAD);
            }
        }
        try {
