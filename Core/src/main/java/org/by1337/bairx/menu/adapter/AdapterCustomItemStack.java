...
import org.by1337.bairx.menu.click.Click;
...
public class AdapterCustomItemStack implements ClassAdapter<MenuItemBuilder> {
    ...
    public MenuItemBuilder deserialize(YamlContext context) {
        ...
        List<PotionEffect> effects = context.getList("potion_effects", String.class, new ArrayList<>()).stream().map(str -> {
            ...
            if (args.length != 3) {
                BAirDropX.getMessage().error("ожидался <PotionEffectType>;<duration>;<amplifier>, а не " + str);
                return null;
            }
        ...
        List<EnchantmentBuilder> enchantments = context.getList("enchantments", String.class, new ArrayList<>()).stream().map(str -> {
                    String[] args = str.split(";");
                    if (args.length != 2) {
                        BAirDropX.getMessage().error("ожидался enchantmentid;level, а не " + str);
                        return null;
                    }
        ...
        MenuItemBuilder menuItemBuilder = new MenuItemBuilder(slots, Collections.unmodifiableList(lore), displayName, clicks, amount, material);
